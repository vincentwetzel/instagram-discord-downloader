...
## [Unreleased]
### Added
- Refreshed the project documentation to describe the current one-account run
  model, slash-command recovery flow, and Discord report truncation behavior
  more clearly across the README and architecture guide.
...
- Single-instance socket lock in `instagram_discord_downloader_bot.py` to prevent multiple
...
- Enforced strict Google-style docstrings across Discord command handlers in
  `instagram_discord_downloader_bot.py`.
...
- Simplified Instagram authentication to use one configured account per run
  from the currently active Firefox session.
...
- Fixed a bug where `ConnectionException` (e.g., rate limit triggers) would be
  ...
  session early.
- Fixed pyright type checking errors in `instagram_discord_downloader_bot.py` (channel send
...
### Added
- `instagram_discord_downloader_bot.py` to handle Discord commands (`!download`).
